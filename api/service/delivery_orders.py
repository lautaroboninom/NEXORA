...
def list_delivery_orders(filters: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if q:
        ...
        where.append(
            """
            (
              o.order_number ILIKE %s OR o.customer_name ILIKE %s OR o.raw_pedido ILIKE %s
              OR o.remito_number ILIKE %s OR o.invoice_number ILIKE %s
              OR o.equipment_serial ILIKE %s OR o.equipment_internal_number ILIKE %s
            )
            """
        )
    ...
    with connection.cursor() as cur:
        cur.execute(
            ...
            [
                ...
                _optional_text(payload.get("equipmentSerial")),
        ...
        for idx, item in enumerate(items):
            ...
            cur.execute(
                ...
                [
                    ...
                    _optional_text(item.get("partida")),
